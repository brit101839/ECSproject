...
public:
    void update(AIContext& ai) override {
        ...
        ai.getMovement()->setDirection(direction);
...
public:
    void update(AIContext& ai) override {
        ...
        ai.getMovement()->setDirection((defaultPos - ai.getTransform()->position).normalize());
        if (ai.getMovement()->getDirection().x >= 0) { ai.getSprite()->setAnimate("walkR"); }
