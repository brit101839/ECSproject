...
Enemy* EnemyManager::addEnemy(const Vector2D& position, const char* spritePath)
{
   /* Entity& enemyEntity = _manager.addEntity();
    ...
    Enemy* enemy = new Enemy("Minotaur", defaultSkill, _globalEventManager);

    Entity* enemyEntity = EnemyFactory::createEnemyFromJson(_manager, "Minotaur", position, enemy->getLocalEventSys());
