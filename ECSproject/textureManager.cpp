...
GLuint TextureManager::SpriteManager(const char* filename, int& width, int& height)
{
	GLuint tmpBufferID;
	auto it = _tileSpriteManager.find(filename);

	if (it != _tileSpriteManager.end()) {
		std::cout << "exited: " << filename << std::endl;
		tmpBufferID = it->second;
		return tmpBufferID;
	}
	else {
		tmpBufferID = loadAndBufferImage(filename, width, height);
		_tileSpriteManager.insert(std::pair<const char*, GLuint>(filename, tmpBufferID));
		std::cout << "Inserted file: " << filename << ", Map size: " << _tileSpriteManager.size() << std::endl;
		return tmpBufferID;
	}
}
